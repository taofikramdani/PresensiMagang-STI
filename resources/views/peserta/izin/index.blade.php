...
@section('content')
    <div class="space-y-6 p-4">
        ...
        <div id="formPengajuan" class="bg-white rounded-xl shadow-sm border border-gray-200 p-6" style="display: none;">
            ...
            <form method="POST" action="{{ route('peserta.izin.store') }}" enctype="multipart/form-data" class="space-y-4" id="izinForm">
                @csrf
    ...
    <div id="editModal" class="fixed inset-0 bg-opacity-10 backdrop-blur-sm hidden z-50">
        <div class="flex items-center justify-center min-h-screen p-4" onclick="event.stopPropagation()">
            <div class="bg-white rounded-xl shadow-2xl border-2 border-gray-200 max-w-md w-full p-6 relative z-10">
                <div class="flex items-center justify-between mb-4">
    ...
    <div id="detailModal" class="fixed inset-0 bg-black/40 backdrop-blur-2xl hidden z-50 flex items-center justify-center"
        onclick="closeDetailModal()">
        <!-- Konten Modal -->
        <div class="bg-white rounded-xl shadow-2xl border border-gray-200 max-w-md w-full p-6 relative z-50"
            ...
            <div class="mt-6">
                <button onclick="closeDetailModal()"
                    class="w-full bg-gray-600 text-white py-2 md:py-3 rounded-xl hover:bg-gray-700 text-sm md:text-base">
                    Tutup
                </button>
            </div>
        </div>
    ...
    <div id="buktiDokumenModal" class="fixed inset-0 bg-black bg-opacity-30 backdrop-blur-sm overflow-y-auto h-full w-full hidden z-50">
        <div class="flex items-center justify-center min-h-screen p-4" onclick="event.stopPropagation()">
            <div class="relative mx-auto border w-full max-w-4xl max-h-[90vh] shadow-2xl rounded-lg bg-white overflow-hidden z-10">
                <!-- Header -->
...
@push('scripts')
<style>
    /* Ensure modals are always clear and clickable */
    #editModal .bg-white,
    #detailModal .bg-white,
    #buktiDokumenModal .bg-white {
    ...
    /* Prevent blur effect on modal content */
    #editModal *,
    #detailModal *,
    #buktiDokumenModal * {
        filter: none !important;
...
<script>
    ...
    // Blur effect functions - Simplified since modals have built-in backdrop
    function addBlurEffect() {
        // No additional blur needed, modals have backdrop-blur-sm
        // Just disable interaction with main content
    ...
    document.addEventListener('DOMContentLoaded', function() {
        // Check if there are validation errors for pengajuan presensi
        const hasPresensiErrors = @json($errors->has('jenis_pengajuan') || $errors->has('tanggal_presensi') || $errors->has('jam_masuk') || $errors->has('jam_keluar') || $errors->has('penjelasan'));
        const hasPresensiSuccess = @json(session('success_presensi') ? true : false);
        
        // Show appropriate tab based on errors or success message
        if (hasPresensiErrors || hasPresensiSuccess) {
            switchTab('presensi'); // Show presensi tab if there are presensi-related errors or success
            if (hasPresensiErrors) {
    ...
    function editPerizinan(id, jenis, tanggal, keterangan) {
        ...
        // Add blur effect to main content only (not modals)
        addBlurEffect();
    }
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Show izin form if there are validation errors for izin (but not for presensi)
        const hasIzinErrors = @json($errors->has('jenis') || $errors->has('tanggal') || $errors->has('keterangan') || $errors->has('bukti_dokumen'));
        @if($errors->any())
            if (hasIzinErrors && !hasPresensiErrors) {
                toggleForm();
            }
        @endif
        ...
        // Show success message and hide form for Izin/Sakit
        @if(session('success'))
            const form = document.getElementById('formPengajuan');
        ...
        // Show success message and hide form for Pengajuan Presensi
        @if(session('success_presensi'))
            if (hasPresensiSuccess) {
                setTimeout(() => {
                    togglePresensiForm();
                }, 3000); // Hide form after 3 seconds
            }
    ...
    function showBuktiDokumen(url, type) {
        ...
        // No additional blur needed, modal has backdrop-blur-sm
        // Just disable interaction with main content
        const mainContainer = document.querySelector('.space-y-6');
        if (mainContainer) {
            mainContainer.style.pointerEvents = 'none';
    ...
    document.addEventListener('keydown', function(e) {
        if (e.key === 'Escape') {
            const buktiModal = document.getElementById('buktiDokumenModal');
            const editModal = document.getElementById('editModal');
            const detailModal = document.getElementById('detailModal');
            
            if (!buktiModal.classList.contains('hidden')) {
                closeBuktiDokumenModal();
