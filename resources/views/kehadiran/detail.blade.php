...
<div class="space-y-4 p-4">
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
        <div class="flex items-center justify-between mb-3">
            <div>
                <a href="{{ route('admin.kehadiran.index') }}" class="inline-flex items-center text-blue-600 hover:text-blue-800 mb-2">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
        @if($presensi->jam_masuk && $presensi->jam_keluar)
            ...
            <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-3">
                <div class="text-center">
                    ...
                    <p class="text-sm font-bold text-purple-600">{{ $durasi->format('%h jam %i menit') }}</p>
                </div>
            </div>
        @endif
        ...
        @if($presensi->jam_masuk && $presensi->jamKerja)
            @php
                $jamMasukTarget = \Carbon\Carbon::parse($presensi->jamKerja->jam_masuk);
                $jamMasukActual = \Carbon\Carbon::parse($presensi->jam_masuk);
                $keterlambatan = $jamMasukActual->diffInMinutes($jamMasukTarget, false);
            @endphp
            @if($keterlambatan > 0)
                <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-3">
                    <div class="text-center">
                        ...
                        <p class="text-sm font-bold text-yellow-600">{{ $keterlambatan }} menit</p>
                    </div>
                </div>
            @endif
        @endif
...
<script>
    ...
    @if($presensi->latitude_masuk || $presensi->latitude_keluar)
        ...
        function initMap() {
            ...
            @if($presensi->lokasi && $presensi->lokasi->latitude && $presensi->lokasi->longitude)
                ...
                L.circle([{{ $presensi->lokasi->latitude }}, {{ $presensi->lokasi->longitude }}], {
                    ...
                    fillColor: '#dc2626',
                    fillOpacity: 0.2,
                    radius: {{ $presensi->lokasi->radius ?? 100 }},
                    weight: 2,
                    dashArray: '5, 5'
                }).addTo(map);
            @endif
