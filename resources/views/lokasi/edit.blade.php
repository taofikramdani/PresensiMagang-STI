        ...
        <div class="bg-white rounded-lg shadow-sm p-6">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-gray-600 mt-2">Edit informasi dan koordinat lokasi presensi</p>
                </div>
                <a href="{{ route('admin.lokasi.index') }}" 
                   class="bg-gray-600 hover:bg-gray-700 text-white font-semibold px-6 py-3 rounded-lg shadow-md transition duration-200 ease-in-out flex items-center space-x-2">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div class="bg-white rounded-lg shadow-sm">
                <div class="p-4 border-b border-gray-200">
                    ...
                    <div class="p-6">
                        <form action="{{ route('admin.lokasi.update', $lokasi->id) }}" method="POST" id="lokasiForm">
                            @csrf
                            @method('PUT')
                            ...
                            <div class="mb-6">
                                ...
                                <select id="is_active" 
                                        name="is_active" 
                                        class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 @error('is_active') border-red-500 @enderror"
                                        required>
                                    <option value="1" {{ old('is_active', $lokasi->is_active) == 1 ? 'selected' : '' }}>Aktif</option>
                                    <option value="0" {{ old('is_active', $lokasi->is_active) == 0 ? 'selected' : '' }}>Tidak Aktif</option>
                                </select>
                                @error('is_active')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            ...
                            <div class="mb-6">
                                ...
                                <textarea id="keterangan" 
                                          ...
                                          placeholder="Keterangan tambahan (opsional)">{{ old('keterangan', $lokasi->keterangan) }}</textarea>
                            </div>
                            ...
                            <div class="mb-6 p-4 bg-blue-50 border border-blue-200 rounded-lg">
                                <div class="flex items-start">
                                    ...
                                    <div class="text-sm text-blue-700">
                                        <p class="font-medium mb-1">Cara mengubah lokasi:</p>
                                        <ul class="text-xs space-y-1">
                                            <li>• Klik pada peta di sebelah kanan untuk menentukan koordinat baru</li>
                                            <li>• Seret marker biru untuk menyesuaikan posisi</li>
                ...
                <div class="bg-white rounded-lg shadow-sm">
                    <div class="p-4 border-b border-gray-200">
                        ...
                        <p class="text-sm text-gray-600 mt-1">Klik di peta untuk mengubah koordinat lokasi</p>
                    </div>
                    
                    <!-- Map Container -->
                    <div class="relative">
                        <div id="map" class="h-[calc(100vh-200px)] min-h-[400px]"></div>
                        ...
                        <div id="coordinates-display" class="absolute bottom-4 left-4 bg-white p-2 rounded-lg shadow-md border">
                            <div class="text-xs text-gray-600">
                                <div>Lat: <span id="display-lat" class="font-mono">{{ $lokasi->latitude }}</span></div>
                                <div>Lng: <span id="display-lng" class="font-mono">{{ $lokasi->longitude }}</span></div>
                            </div>
                        </div>
                    </div>
                </div>
...
<script>
    ...
    const originalLat = {{ $lokasi->latitude }};
    const originalLng = {{ $lokasi->longitude }};
    const originalRadius = {{ $lokasi->radius }};
    ...
    function initializeMap() {
        map = L.map('map').setView([originalLat, originalLng], 16);
    ...
    function addMarker(lat, lng) {
        ...
        const radius = parseInt(radiusInput ? radiusInput.value : originalRadius) || originalRadius;
        ...
        marker.bindPopup(`
            <div class="text-sm">
                <h4 class="font-semibold mb-1">{{ $lokasi->nama_lokasi }}</h4>
    ...
    function setupEventListeners() {
        ...
        // Reset to original
        const resetBtn = document.getElementById('resetToOriginal');
        if (resetBtn) {
            resetBtn.addEventListener('click', function() {
                ...
                const radiusInput = document.getElementById('radius');
                if (radiusInput) {
                    radiusInput.value = originalRadius;
                    updateCircleRadius();
                }
            });
        }
